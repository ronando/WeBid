	...
	function add_error($error)
   {
	...
	function sendmail()
	{
		global $CHARSET, $system;
			$this->errors = array();
			// print_array($this->from);
			 if (!isset($this->from) || empty($this->from))
		{
			$this->from = $system->SETTINGS['adminmail'];
		}
			// print_array($this->from);
			 if ($system->SETTINGS['adminmail'] == $this->to || 1==1) {
	   ...
	   switch ($system->SETTINGS['mail_protocol']) {
		...
		case '3':
		    // do not send email
			trigger_error('---->PHPMailer message: Sending of emails is disabled by Admin' );
	    ...
	    case '1': 
			$mail = new PHPMailer(true);
			$mail->isMail();
			//trigger_error('---->PHPMailer error: isMail');
		break;
